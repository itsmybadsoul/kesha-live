...
export function LivePriceTicker() {
  ...
  return (
    <div className="sticky top-0 z-[100] bg-slate-50 dark:bg-[#0A0A0B] border-b border-slate-200 dark:border-gray-800 overflow-hidden h-10 flex items-center group shadow-xl">
      <div className="flex animate-marquee whitespace-nowrap py-2 hover:pause-marquee">
        {/* Double the list for seamless looping */}
        {[...prices, ...prices].map((p, i) => (
          <div key={`${p.symbol}-${i}`} className="inline-flex items-center gap-2 px-6 border-r border-slate-200 dark:border-gray-800/50">
            <span className="text-[10px] font-black text-slate-400 dark:text-gray-500 uppercase tracking-tighter">{p.symbol}</span>
            <span className="text-xs font-bold text-slate-900 dark:text-white tabular-nums">${p.price}</span>
