...
export function MysteryBox() {
  ...
  return (
    <a href={(isUnlocked || user.hasOpenedMysteryBox) ? "#" : "/deposit"} onClick={handleOpen} className="block group">
      <div className={`bg-gradient-to-br from-[#1A1B1E] to-[#0D0E12] border ${user.hasOpenedMysteryBox ? 'border-slate-200 dark:border-gray-800 opacity-70' : isUnlocked ? 'border-fuchsia-500/50 shadow-[0_0_30px_rgba(217,70,239,0.2)]' : 'border-indigo-500/20'} rounded-3xl p-5 relative overflow-hidden transition-all hover:border-indigo-500/40 hover:shadow-[0_0_30px_rgba(79,70,229,0.15)] active:scale-[0.98]`}>
        ...
        <div className="flex items-center gap-5 relative z-10">
          ...
          <div className="relative shrink-0">
            ...
            {/* Status Badge */}
            <div className="absolute -bottom-1 -right-1 bg-white dark:bg-gray-900 border border-indigo-500/30 rounded-full px-1.5 py-0.5 text-[8px] font-black text-indigo-400 uppercase tracking-tighter">
              Epic
            </div>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-slate-500 dark:text-gray-400 leading-snug line-clamp-2">
              Unlock a random bonus up to <span className="text-slate-900 dark:text-white font-bold">$1,000 USDT</span> when you deposit <span className="text-amber-400 font-bold">$500+</span>.
            </p>
          </div>
          ...
          <div className="shrink-0 flex flex-col items-center gap-1">
             <div className={`w-10 h-10 rounded-full bg-white dark:bg-gray-900/80 border ${user.hasOpenedMysteryBox ? 'border-slate-200 dark:border-gray-800' : isUnlocked ? 'border-fuchsia-500' : 'border-slate-300 dark:border-gray-700'} flex items-center justify-center group-hover:border-indigo-500/50 transition-colors`}>
                {opening ? (
                  ...
                  <LockKeyhole className="w-4 h-4 text-slate-400 dark:text-gray-500 group-hover:text-indigo-400 transition-colors" />
                )}
             </div>
             <span className={`text-[8px] font-black uppercase tracking-widest ${user.hasOpenedMysteryBox ? 'text-emerald-500' : isUnlocked ? 'text-fuchsia-400' : 'text-slate-400 dark:text-gray-500 group-hover:text-indigo-400'}`}>
        ...
        <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-200 dark:border-white/5 flex items-center gap-3">
           ...
           <span className="text-[9px] font-bold text-slate-400 dark:text-gray-500">${balance.toLocaleString()} / $500</span>
        </div>
      </div>
    </a>
  );
