    ...
    <div className="bg-white dark:bg-white dark:bg-gray-900/80 border border-slate-200 dark:border-gray-800 rounded-3xl p-6 shadow-2xl flex flex-col relative overflow-hidden">
      ...
      <div className="flex justify-between items-center mb-6 relative z-10">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-bold text-slate-500 dark:text-gray-400 uppercase tracking-widest">Live Network</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2 space-y-3 scrollbar-none relative z-10 block">
        {liveBets.map((bet) => {
          ...
          return (
            <div key={bet.id} className="group bg-black/40 border border-slate-200 dark:border-gray-800 hover:border-indigo-500/30 rounded-2xl p-4 transition-all hover:shadow-[0_0_20px_rgba(99,102,241,0.1)] animate-in slide-in-from-top-4 fade-in duration-500">
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                  <div className="relative">
                    <img src={bet.trader.avatar} alt="trader" className="w-10 h-10 rounded-full bg-slate-100 dark:bg-gray-800 p-0.5" />
                    {bet.trader.vip && (
                      <div className="absolute -bottom-1 -right-1 bg-amber-500 p-0.5 rounded-full border border-gray-900">
                        <Zap className="w-2.5 h-2.5 text-slate-900 dark:text-white" fill="currentColor" />
                      </div>
                    )}
                  </div>
                  <div>
                    <div className="text-xs font-bold text-slate-600 dark:text-gray-300 flex items-center gap-1">
                      {bet.trader.name}
                      {bet.trader.vip && <span className="text-[9px] bg-amber-500/10 text-amber-500 px-1 rounded uppercase tracking-wider">Pro</span>}
                    </div>
                    <div className="text-[10px] text-slate-400 dark:text-gray-500 font-mono mt-0.5">Placed a bet</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-sm font-black text-slate-900 dark:text-white">${bet.amount.toLocaleString()}</div>
                  <div className="text-[9px] font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest">{bet.duration}M Lock</div>
                </div>
              </div>
              ...
              <div className="bg-white/80 dark:bg-white dark:bg-gray-900/50 rounded-xl p-3 flex justify-between items-center border border-slate-200 dark:border-gray-800/50 group-hover:border-indigo-500/20 transition-colors">
                <div className="flex items-center gap-3">
                   ...
                   <div>
                     <div className="text-sm font-black text-slate-900 dark:text-white">{bet.asset}/USDT</div>
                ...
                <button 
                  ...
                  className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 active:scale-95 transition-all text-slate-900 dark:text-white text-[10px] font-black uppercase tracking-widest rounded-lg flex items-center gap-1 shadow-lg shadow-indigo-500/20"
                >
