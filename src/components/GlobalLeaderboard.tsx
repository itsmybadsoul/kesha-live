...
export function GlobalLeaderboard() {
  ...
  const getRankBadge = (rank: number) => {
    ...
    if (rank === 2) return <Medal className="w-5 h-5 text-slate-600 dark:text-gray-300" fill="currentColor" />;
    if (rank === 3) return <Medal className="w-5 h-5 text-amber-600" fill="currentColor" />;
    return <span className="text-sm font-black text-slate-400 dark:text-gray-500 w-5 text-center">#{rank}</span>;
  };
    ...
    <div className="bg-white dark:bg-white dark:bg-gray-900/80 border border-slate-200 dark:border-gray-800 rounded-3xl p-6 shadow-2xl flex flex-col relative overflow-hidden">
      ...
      <div className="flex justify-between items-center mb-6 relative z-10">
        <div>
          ...
          <p className="text-[10px] text-slate-500 dark:text-gray-400 font-bold uppercase tracking-widest mt-1">Weekly PNL Leaderboard</p>
        </div>
      </div>

      <div className="flex-1 space-y-3 relative z-10">
        {TOP_TRADERS.map((trader) => (
          <div key={trader.rank} className="flex items-center justify-between p-3 bg-black/40 border border-slate-200 dark:border-gray-800 rounded-2xl hover:border-slate-300 dark:border-gray-700 transition-colors">
            <div className="flex items-center gap-3">
              <div className="w-8 flex justify-center shrink-0">
                {getRankBadge(trader.rank)}
              </div>
              <img src={trader.avatar} alt={trader.name} className="w-10 h-10 rounded-full bg-slate-100 dark:bg-gray-800 p-0.5" />
              <div>
                <div className="text-sm font-bold text-slate-600 dark:text-gray-300 flex items-center gap-1">
                  {trader.name}
        ...
        {user && (
          <div className="mt-6 pt-4 border-t border-slate-200 dark:border-gray-800/50">
            <div className="flex items-center justify-between p-4 bg-indigo-500/10 border border-indigo-500/30 rounded-2xl shadow-[0_0_20px_rgba(99,102,241,0.1)] relative overflow-hidden">
              ...
              <div className="flex items-center gap-3 relative z-10">
                ...
                <img src={user.avatar} alt="You" className="w-10 h-10 rounded-full bg-white dark:bg-gray-900 p-0.5 border border-indigo-500/50" />
                <div>
                  <div className="text-sm font-black text-slate-900 dark:text-white">You</div>
