  ...
  if (user.kycStatus !== "VERIFIED") {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-[#0A0A0B] flex items-center justify-center p-4">
        <div className="bg-white/60 dark:bg-slate-100 dark:bg-gray-800/40 backdrop-blur-xl border border-slate-300 dark:border-gray-700/50 rounded-3xl p-8 max-w-md w-full text-center shadow-2xl">
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0A0A0B] text-slate-900 dark:text-white p-4 md:p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white/60 dark:bg-slate-100 dark:bg-gray-800/40 backdrop-blur-xl border border-slate-300 dark:border-gray-700/50 rounded-3xl p-6 md:p-10 shadow-2xl">
          <header className="mb-10">
            ...
            <p className="text-slate-500 dark:text-gray-400 font-medium">Safe and encrypted funds settlement to your preferred method.</p>
          </header>

          <div className="mb-10 bg-indigo-500/5 border border-indigo-500/20 rounded-2xl p-6 flex items-center justify-between">
             <div className="space-y-1">
                <span className="text-[10px] font-black text-slate-400 dark:text-gray-500 uppercase tracking-widest">Available for Settlement</span>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 dark:text-gray-400 uppercase tracking-widest ml-1">Withdrawal Amount (USDT)</label>
              <div className="relative">
                 <input
                   ...
                   className="w-full bg-white dark:bg-gray-900 border border-slate-300 dark:border-gray-700 focus:border-indigo-500 rounded-2xl px-5 py-4 text-slate-900 dark:text-white focus:outline-none transition-all text-lg font-bold"
                   required
                 />
                 <button 
                   type="button"
            ...
            <div className="space-y-6">
              {method === "CRYPTO" ? (
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 dark:text-gray-400 uppercase tracking-widest ml-1">USDT (TRC20) Wallet Address</label>
                  <input
                    type="text"
                    placeholder="Enter your TRC20 destination address"
                    value={cryptoAddress}
                    onChange={(e) => setCryptoAddress(e.target.value)}
                    className="w-full bg-white dark:bg-gray-900 border border-slate-300 dark:border-gray-700 focus:border-indigo-500 rounded-2xl px-5 py-4 text-slate-900 dark:text-white focus:outline-none transition-all font-mono text-sm"
                    required
                  />
                </div>
              ) : (
                <>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 dark:text-gray-400 uppercase tracking-widest ml-1">Beneficiary Full Name</label>
                    <input
                      type="text"
                      placeholder="Account Holder Name"
                      value={accountName}
                      onChange={(e) => setAccountName(e.target.value)}
                      className="w-full bg-white dark:bg-gray-900 border border-slate-300 dark:border-gray-700 focus:border-indigo-500 rounded-2xl px-5 py-4 text-slate-900 dark:text-white focus:outline-none transition-all text-sm font-bold"
                      required
                    />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-slate-500 dark:text-gray-400 uppercase tracking-widest ml-1">Bank IBAN</label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-slate-500 dark:text-gray-400 uppercase tracking-widest ml-1">Beneficiary Bank Name</label>
                      <input
                        type="text"
            ...
            <div className="flex flex-col gap-4">
               ...
               <button
                 type="submit"
                 disabled={loading}
                 className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-slate-900 dark:text-white font-black py-5 rounded-2xl transition-all shadow-xl shadow-indigo-500/20 active:scale-[0.98] uppercase tracking-widest text-xs"
               >
